	...
	# Add all the tournaments as potential start tournaments
        ...
        for i in range(num_tournaments-1):
                for tour in tours_list[i]:
                        for edge in tour[i].edges:
                                tours_list[i+1].append(tour.copy().add_node(edge.node, edge.weight))
                

                
	# Get best tour
	select_tour_least_distance(tours_list, num_tournaments)
